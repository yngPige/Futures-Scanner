...
def main():
    ...
    if recommendation:
        ...
        if recommendation.get("model") == "FALLBACK_TECHNICAL_INDICATORS":
            logger.info("Using fallback technical indicator-based recommendation system.")
            logger.info(f"Fallback Recommendation: {recommendation['recommendation']}")
            logger.info(f"Risk Assessment: {recommendation['risk']}")
        # Check if it's a regular recommendation without errors
        ...
        # Display entry/exit levels if available (for both LLM and fallback)
        if recommendation and 'entry_price' in recommendation:
            logger.info(f"Entry Price: {recommendation['entry_price']}")

        if recommendation and 'stop_loss' in recommendation:
            logger.info(f"Stop Loss: {recommendation['stop_loss']}")

        if recommendation and 'take_profit' in recommendation:
