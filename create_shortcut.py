#!/usr/bin/env python
"""
Create a Windows shortcut for Blacks Scanner.

This script creates a Windows shortcut (.lnk) file that launches the Blacks Scanner
...
def create_shortcut():
    ...
    path = os.path.join(desktop, "Blacks Scanner.lnk")
    ...
    path = os.path.join(current_dir, "Blacks Scanner.lnk")
    shortcut = shell.CreateShortCut(path)
    shortcut.Targetpath = "cmd.exe"
    shortcut.Arguments = f"/k {python_exe} {terminal_path}"
    shortcut.WorkingDirectory = pybloat_dir
